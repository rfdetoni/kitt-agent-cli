...
The contextual surface has three views:

- **Services**: active instances and their local endpoints.
- **Plugins**: connection plugins reported by reverse-proxy; Enter starts the selected provider.
- **Profiles**: named Chromium profiles. Profile data is credential material and remains owned by reverse-proxy.
