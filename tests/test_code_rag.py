...
def test_semantic_reranker_changes_order_without_repository_scan():
    ...
    concurrent = _candidate("history", "history.py", "with self._lock: database.commit()")
    result = SemanticCandidateReranker(KeywordEmbeddingProvider(), max_candidates=8).rank(
        "fix concurrent history race", [unrelated, concurrent]
    )
...
def test_semantic_failure_is_graceful():
    result = SemanticCandidateReranker(BrokenEmbeddingProvider()).rank(
        "concurrent lock", [_candidate("x", "x.py", "lock")]
    )
...
def test_native_retriever_refuses_python_scanner_fallback():
    ...
    assert retriever.retrieve(
        "find lock", None, working_set_paths=set(), max_results=10,
        context_lines=1, token_budget=500,
    ) == []
...
def test_pipeline_uses_rust_lexical_search_instead_of_index_fts(tmp_path, monkeypatch):
    ...
    try:
        ...
        fake_native = FakeRustEngine([{
            "path": "history.py", "line": 3, "column": 9,
            "text": "        with self._lock:",
            "before": ["    def save(self):"],
            "after": ["            self._conn.commit()"], "score": 1.0,
        }])
        monkeypatch.setattr(index, "search_text", lambda *a, **k: (_ for _ in ()).throw(
            AssertionError("FTS must not run after successful Rust retrieval")
        ))
        pipeline = HybridRetrievalPipeline(
            index, native_engine=fake_native, rag_config=RagConfig(semantic_enabled=False)
        )
        selected = pipeline.retrieve("fix concurrent HistoryDatabase lock", max_tokens=1000)
        assert fake_native.calls == 1
        assert pipeline.last_retrieval_stats["lexical_backend"] == "rust"
        assert any(item.path == "history.py" for item in selected)
    finally:
        index.close()
...
def test_semantic_seed_fallback_uses_existing_index_not_file_scanner(tmp_path):
    ...
    try:
        ...
        pipeline = HybridRetrievalPipeline(
            ...
            rag_config=RagConfig(semantic_enabled=True, embedding_model="fake", semantic_candidate_limit=12),
            embedding_provider=KeywordEmbeddingProvider(),
        )
        selected = pipeline.retrieve("prevent simultaneous updates safely", max_tokens=1000)
