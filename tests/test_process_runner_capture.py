...
def test_head_tail_capture_keeps_1k_prefix_and_8k_tail():
    ...
    assert rendered.endswith(b"Z" * (8 * 1024))
    assert b"KITT capture omitted" in rendered
    assert len(rendered) <= 1024 + (8 * 1024) + 128
...
def test_head_tail_capture_respects_smaller_explicit_limit():
    ...
    assert rendered.endswith(b"T" * 3000) or rendered.endswith(b"T" * 3072)
    assert len(rendered) <= 4096 + 128
