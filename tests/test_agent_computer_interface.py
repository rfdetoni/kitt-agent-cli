...
from kitt.tools.registry import ToolRegistry, runtime_operation_names
...
def test_runtime_operation_catalog_is_generated_from_live_contract():
    ...
    assert runtime_tool["args"]["operation"] == "one of: " + ", ".join(expected)
    assert "authoritative" in runtime_tool["description"].lower()
