...
fn compress_search(raw: &str) -> (String, usize) {
    ...
    if omitted > 0 {
        out.push_str(&format!(
            "… {} additional matches omitted; raw output retained by KITT\n",
            omitted
        ));
    }
    (out, omitted)
...
fn compress_build(raw: &str, success: bool) -> (String, usize) {
    ...
    if selected.is_empty() {
        selected.extend(lines.iter().rev().take(40).rev().map(|x| (*x).to_string()));
    }
    let omitted = lines.len().saturating_sub(selected.len());
    let mut out = selected.join("\n");
    if omitted > 0 {
        out.push_str(&format!(
            "\n… {} routine lines omitted; raw output retained by KITT",
            omitted
        ));
    }
    (out, omitted)
...
pub fn compress(
    ...
    if raw.is_empty() {
        return never_worse(&raw, raw.clone(), family(argv), 0);
    }
    let fam = family(argv);
