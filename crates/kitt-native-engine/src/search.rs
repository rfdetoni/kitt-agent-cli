...
fn estimate_tokens(text: &str) -> usize { (text.chars().count() + 3) / 4 }
...
fn compact_line(line: &str, byte_col: usize, needle_len: usize) -> String {
    ...
    let width = needle_len.max(1).min(80);
    let left = col.saturating_sub(140);
    let right = (col + width + 140).min(chars.len());
    format!("{}{}{}", if left > 0 { "…" } else { "" }, chars[left..right].iter().collect::<String>(), if right < chars.len() { "…" } else { "" })
}
...
fn build_regex(query: &str, opts: &SearchOptions) -> Result<Regex> {
    ...
    RegexBuilder::new(&pattern).case_insensitive(!opts.case_sensitive).build().context("invalid search pattern")
}
...
pub fn search(root: &Path, query: &str, options: SearchOptions) -> Result<SearchResponse> {
    ...
    walker.hidden(!opts.include_hidden).git_ignore(true).git_global(true).git_exclude(true);
    ...
    'files: for entry in walker.build().filter_map(Result::ok) {
        ...
        if bytes.iter().take(8192).any(|b| *b == 0) { continue; }
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if *used >= opts.max_per_file { continue; }
            if hits.len() >= opts.max_results { break 'files; }
            let start = idx.saturating_sub(opts.context_lines);
            let end = (idx + opts.context_lines + 1).min(lines.len());
            let before = lines[start..idx].iter().map(|s| s.to_string()).collect::<Vec<_>>();
            let after = lines[idx + 1..end].iter().map(|s| s.to_string()).collect::<Vec<_>>();
            ...
            let hit_tokens = estimate_tokens(&rendered)
                ...
                + after.iter().map(|x| estimate_tokens(x)).sum::<usize>() + 12;
            if !hits.is_empty() && tokens + hit_tokens > opts.token_budget { break 'files; }
            tokens += hit_tokens;
            *used += 1;
            matched_files.insert(rel.clone());
            hits.push(SearchHit {
                path: rel.clone(), line: idx + 1, column: line[..m.start()].chars().count() + 1,
                text: rendered, before, after,
    ...
    Ok(SearchResponse {
        ...
        matched_files: matched_files.len(), total_matches_seen: total_seen,
        estimated_tokens: tokens, hits,
    })
}
