...
def build_parser() -> argparse.ArgumentParser:
    ...
    # 'daemon' subcommand. ``run`` is deliberately available because the
    # detached process launcher uses it as the private foreground entry point.
    daemon_parser = subparsers.add_parser("daemon", help="Manage persistent background KITT Daemon")
    daemon_parser.add_argument("daemon_action", nargs="?", default="status", choices=["start", "run", "stop", "status"], help="Daemon action")

    # First-party remote web interface. ``web`` is a friendly alias.
    for remote_name in ("remote", "web"):
        remote_parser = subparsers.add_parser(
            remote_name,
            help="Serve the KITT web interface for local/private-network control",
        )
        remote_parser.add_argument("--lan", action="store_true", help="Bind to all interfaces for trusted-LAN access")
        remote_parser.add_argument("--host", default=None, help="Explicit bind host (default: 127.0.0.1 or 0.0.0.0 with --lan)")
        remote_parser.add_argument("--port", type=int, default=7337, help="HTTP port (default: 7337; 0 chooses a free port)")
        remote_parser.add_argument("--pairing-ttl", type=float, default=900.0, help="Pairing-code lifetime in seconds")
        remote_parser.add_argument("--session-ttl", type=float, default=43_200.0, help="Web-session lifetime in seconds")
