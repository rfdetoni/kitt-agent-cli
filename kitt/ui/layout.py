...
def build_root_container(ui):
    ...
    prompt = Frame(Window(ui.prompt_control, height=Dimension(min=3, max=8), wrap_lines=True), title="Prompt  Alt+Enter newline")
    ...
    home = HSplit([
        ...
        Window(ui.home_control, height=9, align=WindowAlign.CENTER),
        Box(Frame(Window(ui.prompt_control, height=Dimension(min=3, max=7), wrap_lines=True), title="Ask K.I.T.T."), padding_left=4, padding_right=4),
    ...
    floats = [
        ...
        Float(content=ConditionalContainer(Box(Frame(HSplit([Window(ui.provider_endpoint_help_control, height=4), Window(ui.provider_endpoint_control, height=1)]), title="Add Ollama Endpoint"), padding=1), filter=visible("provider_endpoint")), left=8, right=8, top=5, height=8),
