...
def _daemon_authoritative(runtime) -> bool:
    config = getattr(runtime, "config", None)
    return bool(
        config
        and getattr(config, "daemon_enabled", False)
        and getattr(config, "history_enabled", False)
        and getattr(config, "persistence_enabled", False)
    )


async def _run_daemon_turn(runtime, prompt: str, write, *, readline=None) -> bool:
    """Run one turn through the daemon and return True when it failed/blocked."""
    from kitt.ui.event_bridge import TurnEventBridge

    conversation = runtime.history.get_or_create_active()
    event_queue: asyncio.Queue = asyncio.Queue()
    bridge = TurnEventBridge(runtime, event_queue.put_nowait, lambda: None)
    full_response = ""
    failed = False
    try:
        # Attach first and discard replayed historical UI events. This helper is
        # for one new foreground turn; old terminal events must not terminate it.
        if not await bridge.ensure_daemon(conversation["id"]):
            raise RuntimeError("Unable to connect to KITT daemon")
        ...
        await bridge.start(
            prompt,
            conversation["id"],
            no_history=not runtime.config.history_enabled,
        )
        while True:
            ...
            if isinstance(event, TextDelta):
                ...
                if readline is None:
                    write("\nApproval required; request denied in non-interactive mode.\n")
                    await bridge.resolve_approval(event.approval_request_id, False)
                    failed = True
                else:
                    write(
                        f"\nApproval required: {event.tool_name} "
                        f"[{event.action_hash[:8]}] (y/N): "
                    )
                    answer = (await asyncio.to_thread(readline)).strip().lower()
                    await bridge.resolve_approval(
                        event.approval_request_id,
                        answer in {"y", "yes"},
                    )
            elif isinstance(event, TurnCompleted):
                full_response = _write_final_response(write, full_response, event.response)
                break
            ...
            if isinstance(event, TextDelta):
                ...
                if answer in {"y", "yes"}:
                    ...
                    async for resumed in _stream_iterator(resumed_iterator):
                        ...
                        elif isinstance(resumed, TurnFailed):
                            self._write(f"\nError: {resumed.error}")
                else:
                    for _ in self.runtime.processor.cancel_turn(
                        event.turn_id,
                        "Approval denied",
                        conversation_id=conversation["id"],
                    ):
...
class HeadlessUI:
    ...
    async def run_async(self) -> int:
        ...
        if _daemon_authoritative(self.runtime):
            return 1 if await _run_daemon_turn(self.runtime, self.prompt, write) else 0
        ...
        async for event in _stream_iterator(iterator):
            if isinstance(event, TextDelta):
                ...
                write(f"Error: {event.error}\n")
                failed = True
            elif isinstance(event, ApprovalRequired):
                write("Approval required; rerun interactively.\n")
                failed = True
